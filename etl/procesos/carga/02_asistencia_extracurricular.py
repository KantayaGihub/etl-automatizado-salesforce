...
def get_batch_results_safe(
    ...
    try:
        ...
        print(
            f"⚠️ Error obteniendo "
            f"resultados batch: {e}"
        )
...
if not records_delete:
    ...
    ids = [
        ...
        for r in records_delete

    ]

    print(
        f"ℹ️ Registros encontrados "
        f"para eliminar: {len(ids):,}"
    )

    delete_job = bulk.create_delete_job(
        OBJECT_NAME,
        contentType="JSON"
    )
    ...
    for i in range(
        ...
        print(
            f"🗑️ Batch delete enviado "
            f"{i // DELETE_BATCH_SIZE + 1}"
        )
...
print(
    f"ℹ️ Filas: {len(Asistencias):,}"
)

print(
    f"ℹ️ Columnas: "
    f"{len(Asistencias.columns)}"
)
...
if "CENTRO" in Asistencias.columns:
    ...
    Asistencias["CENTRO"] = (
        ...
        .str.replace(
            " REGISTRO DE ASISTENCIA 2025.xlsx",
            "",
            regex=False
        )

        .str.replace(
            " REGISTRO DE ASISTENCIA 2026.xlsx",
            "",
            regex=False
        )
...
Asistencias = Asistencias.where(
    pd.notnull(Asistencias),
    None
)
...
Asistencias.columns = [
    ...
    for col in Asistencias.columns
...
sf_fields = {
    ...
    for f in (
        ...
        sf
        .Asistencias_Extracurriculares_e__c
        .describe()["fields"]
...
invalid_cols = [
    ...
    c for c in Asistencias.columns

    if c not in sf_fields
...
if invalid_cols:
    ...
    print(
        "\n⚠️ Campos inexistentes "
        "en Salesforce:"
    )

    print(invalid_cols)

    Asistencias = Asistencias.drop(
...
print(
    f"ℹ️ Registros a insertar: "
    f"{len(records_insert):,}"
)
...
insert_job = bulk.create_insert_job(
    OBJECT_NAME,
    contentType="JSON"
)
...
for i, batch_id in enumerate(
    ...
    resultados = get_batch_results_safe(
        bulk,
        insert_job,
        batch_id
    )

    ok, fail, errores = summarize_results(
        resultados
    )
